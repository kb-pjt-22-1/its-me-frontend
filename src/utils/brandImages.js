...
// brand_logo는 'Brands/starbucks.png'처럼 src/images/ 밑 상대 경로를 그대로 담고 있습니다
// (brand_code에서 슬러그를 유추하지 않음 - mega-mgc-coffee.png나 한글 파일명처럼 규칙이
// 안 맞는 케이스가 있어서, DB에 실제 파일명을 정확히 저장해두고 그대로 매칭합니다).
// 매칭되는 파일이 없으면(값이 비었거나 오타 등) null을 돌려주며, 호출 쪽에서 카테고리
// 아이콘 등으로 폴백하면 됩니다 - 필수값이 아닙니다.
export function getBrandImage(brandLogo) {
  if (!brandLogo) return null
  const entry = Object.entries(brandImageModules).find(([path]) => path.endsWith(`/${brandLogo}`))
  return entry?.[1] ?? null
}
